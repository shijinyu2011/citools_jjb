...
#git clone git@gitlabe1.ext.net.nokia.com:prime/sad-runner.git  -b master --depth 1
...
if [ `echo $?` = 0 ];then 
  for i in `cat cciscript/checkout/dep_ft.config|grep "$subsystem:"|cut -d ':' -f2`;do 
    ...
    $svnenv -j "cd $i/build;make rpm" >> $WORKSPACE/statistics/compile.log
...
if [ "{v}" == "t" ]; then
  ...
  cat $WORKSPACE/$wkdir/$utresult_folder/TESTS-TestSuites.xml|grep -E "^[ |\t]*\<testsuite" | grep -E "(errors|failures)=\"[1-9]+\""
fi
